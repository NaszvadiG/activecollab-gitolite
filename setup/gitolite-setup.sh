...
sudo adduser --system --home /home/$GITUSER --shell /bin/bash --group --disabled-login --disabled-password --gecos 'git version control' $GITUSER || OwnError " Unable to create $GITUSER :("
...
sudo -H -u $GITUSER git clone git://github.com/sitaramc/gitolite || OwnError " Unable to clone gitolote repository :("
...
then
	...
	sudo mv -v $WEBUSERHOME/.ssh/id_rsa $WEBUSERHOME/.ssh/id_rsa.bak
	sudo mv -v $WEBUSERHOME/.ssh/id_rsa.pub $WEBUSERHOME/.ssh/id_rsa.pub.bak
	echo
fi
...
sudo cp -v $WEBUSERHOME/.ssh/id_rsa.pub /home/$GITUSER/$WEBUSER.pub || OwnError " Unable to copy $WEBUSER Pubkey :(" 
...
cd /home/$GITUSER
sudo -H -u $GITUSER /home/$GITUSER/bin/gitolite setup -pk $WEBUSER.pub || OwnError " Unable to setup Gitolite Admin (Key) :("
