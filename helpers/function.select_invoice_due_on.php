<?php
  ...
  function smarty_function_select_invoice_due_on($params, &$smarty) {
    ...
    if($mode == 'select') {
      //if drop down control required
      $control =  HTML::selectFromPossibilities($name, $possibilities, $value, $params); 
    } else {
      //if radio buttons control
      $allow_selected = array_var($params, 'allow_selected', true, true);
      $control =  HTML::radioGroupFromPossibilities($name, $possibilities, $value, $params); 
    }//if
